...
const fetchNewsApiData = async (searchTerm: string) => {
  try {
    ...
    const sources = newsApiSources.data.sources;
    // To create a mapping of sources to their respective categories
    const newsApiCategoriesMap : Record<string, string> = sources.reduce((map: Record<string, string>, source: { id: string; category: string }) => {
